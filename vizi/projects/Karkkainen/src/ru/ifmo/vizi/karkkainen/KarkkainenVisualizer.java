...
{
    /**
     * Автомат алгоритма Кярккайнена
     */
    private final Karkkainen auto;

    /**
     * Данные автомата алгоритма Кярккайнена
     */
    ...
    private void init(char[] str) {
        ...
        //ЏҐаҐ¬®вЄ   «Ј®аЁв¬  ў ­ з «® Ё Ё­ЁжЁ «Ё§ жЁп

        rewind(0);
        data.s = str;
        int size = data.s.length;

        //Џ®бва®Ґ­ЁҐ ўбҐе дЁЈга
        all = new Vector();
        //‚л¤Ґ«Ґ­ЁҐ ­г¦­ле ¤ ­ле ­ҐЇ®баҐ¤бвўҐ­­® Ё§  «Ј®аЁв¬ .
        (new KarkkainenAlgorythm()).suffArray(data);

        //‚лзЁб«Ґ­ЁҐ ­ҐЄ®в®але Є®­бв ­в
        double sx;
...
//        System.out.println("=) 1");
        ...
        for (int i = 0; i < recStrN; i++) {
          //  System.out.println(i);
            ...
            int n12 = n - (n + 2) / 3; //Љ®«ЁзҐбвў® ­г¦­ле бгддЁЄб®ў
